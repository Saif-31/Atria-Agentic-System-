...
class DatabaseManager:
    ...
    def store_session_data(self, session_data: dict) -> str:
        ...
        try:
            ...
            sanitized_data = prepare_document_for_mongodb(session_data)
            ...
            if session_id:
                ...
                if existing_session:
                    ...
                    # Remove _id from session_data to avoid conflict
                    if '_id' in sanitized_data:
                        del sanitized_data['_id']
...
def process_user_input(input_type: str, data: Any) -> Dict:
    ...
    try:
        ...
        api_key = os.getenv('OPENAI_API_KEY')
        if not api_key:
            print("Debug - Environment variables:", {k: v for k, v in os.environ.items() if 'API' in k})
            return {
                "status": "error",
                "message": "OpenAI API key not found. Check that:",
                "details": [
                    "1. Your .env file exists in the correct location",
                    "2. The .env file contains OPENAI_API_KEY=your-key",
                    "3. The .env file is being loaded correctly",
                    f"4. Current working directory: {os.getcwd()}"
                ]
            }
